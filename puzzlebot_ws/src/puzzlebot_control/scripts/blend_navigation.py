...
class GoToGoal(): 
    def __init__(self): 
        ...
        ############ Variables ###############
        self.x_target=6 #x position of the goal
        self.y_target=-3 #y position of the goal
        ...
        self.blended_alpha = 0.7 # blending factor for the blended controller (thet higher the more aggressive obstacle avoidance)
        v_msg=Twist() #Robot's desired speed 
        self.wr=0 #right wheel speed [rad/s]
        self.wl=0 #left wheel speed [rad/s]
        
        self.current_state = 'Goal' #Robot's current state
        rospy.on_shutdown(self.cleanup) 
        ###******* INIT PUBLISHERS *******### 
        ...
        while not rospy.is_shutdown(): 
            ...
            if self.lidar_received:
                ...
                if closest_range > blended_distance:
                	self.current_state = 'Goal'
                elif (closest_range <= blended_distance) and (closest_range > ao_distance):
                	self.current_state = 'Blend'
                elif (closest_range <= ao_distance) and (closest_range > stop_distance):
                	self.current_state = 'AvoidObstacle'
                else:
					self.current_state = 'Stop'
                    
                if self.current_state == 'AvoidObstacle':
                    if self.at_goal(): 
                        self.current_state = 'Stop'
                    else:
                ...
                elif self.current_state == 'Blend':
                    if self.at_goal(): 
                        self.current_state = 'Stop'
                    else:
                        v_blend , w_blend = self.blending()
    ...
    def blending(self):
      	v_gtg, w_gtg = self.compute_gtg_control(self.x_target, self.y_target, self.robot.x, self.robot.y, self.robot.theta)
      	v_ao, w_ao = self.compute_ao_control(self.lidar_msg)
        vblend = self.blended_alpha*v_ao + (1-self.blended_alpha)*v_gtg
        wblend = self.blended_alpha*w_ao + (1-self.blended_alpha)*w_gtg
        return vblend, wblend
